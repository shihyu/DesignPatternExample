...
Plus* Plus_construct(void* addr, IExpression* leftExpression,
...
int Plus_interpret(IExpression* iexpression)
{
    ...
    return plus->leftExpression->interpret(plus->leftExpression) +
           plus->rightExpression->interpret(plus->rightExpression);
}
