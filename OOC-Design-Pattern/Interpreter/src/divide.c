...
Divide* Divide_construct(void* addr, IExpression* leftExpression,
...
int Divide_interpret(IExpression* iexpression)
{
    ...
    return divide->leftExpression->interpret(divide->leftExpression) /
           divide->rightExpression->interpret(divide->rightExpression);
}
