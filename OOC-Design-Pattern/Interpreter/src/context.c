...
int Context_calculate(Context* context, char* suffixExpression)
{
    ...
    while (token != NULL) {
        ...
        if (first == '+' || first == '-' || first == '*' || first == '/') {
            ...
            if (strcmp(token, "+") == 0) {
                context->expressions[context->count++] = &new(Plus, leftExpression,
                        rightExpression)->iexpression;
            } else if (strcmp(token, "-") == 0) {
                context->expressions[context->count++] = &new(Minus, leftExpression,
                        rightExpression)->iexpression;
            } else if (strcmp(token, "*") == 0) {
                context->expressions[context->count++] = &new(Multiply, leftExpression,
                        rightExpression)->iexpression;
            } else if (strcmp(token, "/") == 0) {
                context->expressions[context->count++] = &new(Divide, leftExpression,
                        rightExpression)->iexpression;
            } else {
            ...
            if (context->count == context->size) {
                ...
                memcpy(expressions, context->expressions,
                       context->count * sizeof(IExpression*));
                // TODO delete
