...
Minus* Minus_construct(void* addr, IExpression* leftExpression,
...
int Minus_interpret(IExpression* iexpression)
{
    ...
    return minus->leftExpression->interpret(minus->leftExpression) -
           minus->rightExpression->interpret(minus->rightExpression);
}
