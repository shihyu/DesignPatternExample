...
Multiply* Multiply_construct(void* addr, IExpression* leftExpression,
...
int Multiply_interpret(IExpression* iexpression)
{
    ...
    return multiply->leftExpression->interpret(multiply->leftExpression) *
           multiply->rightExpression->interpret(multiply->rightExpression);
}
