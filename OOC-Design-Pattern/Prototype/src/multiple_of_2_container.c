...
MultipleOf2Container* MultipleOf2Container_construct(void* addr, int size)
{
    ...
    AbstractNumberContainer_construct(
        &multipleOf2Container->abstractNumberContainer, size);
...
void MultipleOf2Container_destruct(MultipleOf2Container* multipleOf2Container)
{
    AbstractNumberContainer_destruct(
        &multipleOf2Container->abstractNumberContainer);
}

IClone* MultipleOf2Container_clone(IClone* iclone)
{
    MultipleOf2Container* multipleOf2Container = container_of(iclone,
            MultipleOf2Container, iclone);
    MultipleOf2Container* newContainer = new(MultipleOf2Container,
            multipleOf2Container->size);
    memcpy(newContainer->container, multipleOf2Container->container,
           sizeof(int) * multipleOf2Container->idx);
    newContainer->idx = multipleOf2Container->idx;
    return &newContainer->iclone;
}

int MultipleOf2Container_align(AbstractNumberContainer* abstractNumberContainer,
                               int number)
{
