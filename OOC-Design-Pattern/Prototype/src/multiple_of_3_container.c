...
MultipleOf3Container* MultipleOf3Container_construct(void* addr, int size)
{
    ...
    AbstractNumberContainer_construct(
        &multipleOf3Container->abstractNumberContainer, size);
...
{
    AbstractNumberContainer_destruct(
        &multipleOf3Container->abstractNumberContainer);
}

IClone* MultipleOf3Container_clone(IClone* iclone)
{
    MultipleOf3Container* multipleOf3Container = container_of(iclone,
            MultipleOf3Container, iclone);
    MultipleOf3Container* newContainer = new(MultipleOf3Container,
            multipleOf3Container->size);
    memcpy(newContainer->container, multipleOf3Container->container,
           sizeof(int) * multipleOf3Container->idx);
    newContainer->idx = multipleOf3Container->idx;
    return &newContainer->iclone;
}

int MultipleOf3Container_align(AbstractNumberContainer* abstractNumberContainer,
                               int number)
{
    number = number + 2;
