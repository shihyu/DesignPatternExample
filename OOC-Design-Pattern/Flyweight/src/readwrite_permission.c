...
ReadWritePermission* ReadWritePermission_construct(void* addr,
        IPermission* permission1, IPermission* permission2)
{
    if (addr == NULL) {
        return NULL;
    }
...
{
    ReadWritePermission* readWritePermission = container_of(ipermission,
            ReadWritePermission, ipermission);
    IPermission** permissions = readWritePermission->permissions;
    return permissions[0]->match(permissions[0], permission) ||
           permissions[1]->match(permissions[1], permission);
}
