...
void CarElementRunVisitor_visitWheel(ICarElementVisitor* icarElementVisitor,
                                     Wheel* wheel)
{
    printf("Visit %s!\n", wheel->getName(&wheel->icarElement));
}

void CarElementRunVisitor_visitEngine(ICarElementVisitor* icarElementVisitor,
                                      Engine* engine)
{
    printf("Visit %s!\n", engine->getName(&engine->icarElement));
}

void CarElementRunVisitor_visitBody(ICarElementVisitor* icarElementVisitor,
                                    Body* body)
{
