...
ProjectManager* ProjectManager_construct(void* addr)
{
    ...
    projectManager->handleAskForLeaveRequest =
        ProjecttManager_handleAskForLeaveRequest;
...
int ProjecttManager_handleAskForLeaveRequest(IManager* imanager, char* name,
        int days)
{
    if (days < 7) {
    ...
    ProjectManager* projectManager = container_of(imanager, ProjectManager,
                                     imanager);
    ...
    return projectManager->nextManager->handleAskForLeaveRequest(
               projectManager->nextManager, name, days);
}

int ProjectManager_handleRaisesRequest(IManager* imanager, char* name,
                                       int money)
{
    if (money > 10000) {
    ...
    if (projectManager->nextManager != NULL) {
        return projectManager->nextManager->handleRaisesRequest(
                   projectManager->nextManager, name, money);
    }
